...
methodHandle SharedRuntime::reresolve_call_site(bool& is_static_call, bool& is_optimized, bool& caller_does_not_scalarize, TRAPS) {
  ...
  // Do nothing if the frame isn't a live compiled frame. nmethod could be deoptimized by the time we get here so no
  // update to the caller is needed. However, when the caller is C2 compiled and the callee a C1 or C2 compiled method
  // with an inline type receiver, then we still need to figure out whether it was a static or an optimized virtual call.
  // Otherwise, we end up using the wrong method entry point and accidentally skip the buffering of the receiver.
  bool callee_compiled_with_scalarized_receiver = false;
  if (callee_method->has_compiled_code() && !callee_method()->is_static() && callee_method()->is_scalarized_arg(0)) {
    callee_compiled_with_scalarized_receiver = true;
  }
  const bool do_IC_clearing = (caller.is_compiled_frame() && !caller.is_deoptimized_frame()) ||
